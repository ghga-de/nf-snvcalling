...
usage() { echo "Usage: $0 [-p prefix] [-i vcf_file] [-t tumor] [-v basequal] [-b background] [-o output] [-r force] [-m median_filter_threshold] [-w title] [-sp skipplots] [-rb refBaseQual] [-ab altBaseQual] [-ar altReadPos] [-rr refReadPos]" 1>&2; exit 1; }
...
SNV_FILE_WITH_MAF=${prefix}.withMAF.vcf
...
tripletBased_BQDistribution_plotter.R -v ${SNV_FILE_WITH_MAF} -a ${FILENAME_TUMOR_BAM} \
        -p ${prefix} -b ${background} -o ${output} -R ${force} -c 1 \
        -f ${median_filter_threshold} -s ${SEQUENCE_CONTEXT_COLUMN_INDEX} --MAFColumnIndex ${MAF_COLUMN_INDEX} -i 1 \
